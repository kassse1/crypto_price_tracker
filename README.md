...
Backend-сервис для периодического получения и хранения index price криптовалют с биржи Deribit.

Сервис каждую минуту получает текущую цену `btc_usd` и `eth_usd` через публичный Deribit API, сохраняет данные в PostgreSQL и предоставляет REST API для получения последней цены и истории за период.
